...
namespace FloorPlanner
{
    public partial class DiagramToolBar
    {
        ...
        /// </summary>
        private void ToolbarEditorClick(Syncfusion.Blazor.Navigations.ClickEventArgs args)
        {
            SfDiagramComponent diagram = Parent.DiagramContent.Diagram;
            string commandType = args.Item.TooltipText.ToLower(new CultureInfo("en-US"));
            switch (commandType)
            {
                case "undo":
                    diagram.Undo();
                ...
                case "horizontal flip":
                    Parent.MenuBar.FlipOptions("Flip Horizontal");
                    break;
                case "vertical flip":
                    Parent.MenuBar.FlipOptions("Flip Vertical");
                    break;
                case "ungroup":
        ...
        public void DeleteData()
        {
            ...
            {
                for (int i = diagram.SelectionSettings.Nodes.Count - 1; i >= 0; i--)
                {
            ...
            {
                for (int i = diagram.SelectionSettings.Connectors.Count - 1; i >= 0; i--)
                {
                    Connector item1 = diagram.SelectionSettings.Connectors[i];
        ...
        /// </summary>
        private void LockObject(bool isPreventPropertyChange = false)
        {
            bool isLock = false;
            SfDiagramComponent diagram = Parent.DiagramContent.Diagram;
            ...
            for (int j = 0; j < diagram.SelectionSettings.Connectors.Count; j++)
            {
                Connector connector = diagram.SelectionSettings.Connectors[j];
        ...
        public void SingleSelectionToolbarItems(bool isRefresh = true)
        {
            ...
            ShowVerticalFlip = diagram ? false : !ShowVerticalFlip ? true : ShowVerticalFlip;
            ShowHorizontalFlip = diagram ? false : !ShowHorizontalFlip ? true : ShowVerticalFlip;
